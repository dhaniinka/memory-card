...
import { RefreshCcw, FolderOpen, Timer, Star, Flame } from "lucide-react";
...
const cardImages = [
  ...
  { src: "/cards/onde.png", matched: false },
  { src: "/cards/apem.png", matched: false },
  { src: "/cards/cendol.png", matched: false },
  { src: "/cards/jenang.png", matched: false },
  { src: "/cards/dawet.png", matched: false },
  { src: "/cards/lemper.png", matched: false },
  { src: "/cards/kueputu.png", matched: false },
  { src: "/cards/cubit.png", matched: false },
  { src: "/cards/talam.png", matched: false },
  { src: "/cards/lapis.png", matched: false },
  { src: "/cards/nastar.png", matched: false },
  { src: "/cards/kastengel.png", matched: false },
];
...
export default function GameHard() {
  ...
  const [timeLeft, setTimeLeft] = useState(120); // kasih waktu lebih lama (2 menit)

  // Shuffle cards saat start
  const shuffleCards = () => {
    const shuffled = [...cardImages, ...cardImages] // jadi 36 kartu
  ...
  useEffect(() => {
    if (choiceOne && choiceTwo) {
      ...
      if (choiceOne.src === choiceTwo.src) {
        ...
        setScore((prev) => prev + 15); // skor lebih tinggi di hard
        resetTurn();
      } else {
        setTimeout(() => {
          setScore((prev) => (prev > 0 ? prev - 3 : 0));
          resetTurn();
        }, 1000);
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center w-full max-w-6xl mb-6">
        ...
        {/* Judul (tengah) */}
        <h1 className="text-4xl font-bold flex items-center gap-2 text-center">
          <Flame className="w-8 h-8 text-red-400" /> Hard Mode
        </h1>
